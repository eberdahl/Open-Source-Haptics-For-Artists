...
int dirPinA = 12;   // Direction control of Motor A 
int PWMpinA = 3;    // PWM speed control of Motor A
int dirPinB = 13;   // Direction control of Motor B 
int PWMpinB = 11;   // PWM speed control of Motor B 
...
int capSensePinA = 4;       // The capacitive sensing "input" is at pin D10.  (USING D9 ON ARDUINO NANO!)
...
int timingCheckPin = 7;     // This pin is used for checking the timing  (WAS 11 FOR NANO)
...
void setup()
{
  ...
  // For the Uno, make sure that the brake pins are not set
  pinMode(8, OUTPUT);
  digitalWrite(8, 0);
  pinMode(9, OUTPUT);
  digitalWrite(9, 0);
  ...
  //TCCR0B = TCCR0B & 0b11111000 | 0x01;   // (set PWM speed as fast as possible for pins 5 and 6 -- see section 14.9.2 "TCCR0B – Timer/Counter Control Register B" in ATMega328 full-length data sheet)
  TCCR2B = TCCR2B & 0b11111000 | 0x01;  // (set PWM speed as fast as possible for pins 3 and 11)
...
inline int setPWM(int PWMpin, int dirPin, int force, int forceEnable){
  forceEnable = 129;   // in case it is a problem with forceEnable
...
void loop()
{
  ...
  analog2 = analogReadAveraging(currentPinB, 8, 3);    // This takes about 150 microseconds
  analog2 = analog2 >> 2;    // For convenience, make the result fit into 8 bits instead of the ADC default 10 bits
  analog1 = analogReadAveraging(currentPinA, 8, 3);    // This takes about 150 microseconds
  ...
  potValueB = analogReadAveraging(potPinB, 16, 4);         // This takes about 300 microseconds
  potValueB = potValueB >> 2;                              // Make the 10-bit average fit into 8 bits for serial comm.
  potValueA = analogReadAveraging(potPinA, 16, 4);         // This takes about 300 microseconds
  potValueA = potValueA >> 2;                              // Make the 10-bit average fit into 8 bits for serial comm.



  
  // Send data to laptop over serial connection -- this takes 0.875 ms
